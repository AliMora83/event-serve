
const Footer = () => {
  return (
    // Replaced entire footer content with updated styles and year
    <footer className="bg-black text-gray-300 py-16 border-t border-gray-800">
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div>
            <h3 className="text-xl font-bold mb-4 text-white">EventServe</h3>
            <p className="text-gray-300 text-sm">Creating memorable events since 2020</p>
          </div>
          <div>
            <h4 className="font-bold mb-4 text-white">Quick Links</h4>
            <ul className="space-y-2">
              <li><a href="#about" className="text-gray-300 hover:text-white transition-colors text-sm">About</a></li>
              <li><a href="#services" className="text-gray-300 hover:text-white transition-colors text-sm">Services</a></li>
              <li><a href="#contact" className="text-gray-300 hover:text-white transition-colors text-sm">Contact</a></li>
            </ul>
          </div>
          <div>
            <h4 className="font-bold mb-4 text-white">Contact</h4>
            <p className="text-gray-300 text-sm">Email: info@eventserve.co.za</p>
            <p className="text-gray-300 text-sm">Phone: 073 463 4425 / 064 050 4442</p>
          </div>
        </div>
        <hr className="border-t border-gray-800 my-8" />
        <p className="text-center text-gray-400 text-sm">Copyright &copy; {new Date().getFullYear()} EventServe. All rights reserved.</p>
      </div>
    </footer>
  );
};
export default Footer;
